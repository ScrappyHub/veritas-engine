...
$cmd = @("python", $ver, "verify", $bundlePath, "--json", "--deterministic")
$actual = & $cmd[0] $cmd[1] $cmd[2] $cmd[3] $cmd[4] $cmd[5] $cmd[6]
if($LASTEXITCODE -ne 0){ throw ("VERIFIER_EXIT_NONZERO: " + $LASTEXITCODE) }
if([string]::IsNullOrWhiteSpace($actual)){ throw "VERIFIER_EMPTY_OUTPUT" }

# Write actual JSON deterministically
...
if( ($b[0] -ne 0x7B) -or ($b[1] -ne 0x22) -or ($b[2] -ne 0x62) ){
  throw ("ACTUAL_HEADER_BYTES_BAD: {0:X2} {1:X2} {2:X2}" -f $b[0],$b[1],$b[2])
}
